## Appendix E: Bayesian data analysis
...
A Bayesian model calculates a posterior distribution, i.e., a distribution of plausible parameter values, given the data, a data-generating model, and any prior information we have about those parameter values.
...
In practical terms, what this means is that we do not calculate a single point estimate for an effect β, but rather we draw a sample of 4,000 plausible values for β.
This allows us to quantify our uncertainty regarding β by summarizing the distribution of those values.
We will use 4 statistics to describe the posterior distribution: (1) the mean, (2) the highest density credible interval (HDI), (3) the proportion of the HDI that falls within a Region of Practical Equivalence (ROPE), and (4) the Maximum Probability of Effect (MPE).
...
[^rope]: We utilize a ROPE of ±1 for standardized values.
    For non-standardized values @kruschke2018rejecting recommends using the formula in @eq-rope.

$$
es = \frac{\mu_1 - \mu_2}{\sqrt{\frac{\sigma_1^2 + \sigma_2^2}{2}}}
$$ {#eq-rope}

If, for instance, a hypothesis states that β \> 0, we judge there to be *compelling evidence* for this hypothesis if the mean point estimate is a positive number, if the 95% credible interval of β does not contain 0 and is outside the ROPE by a reasonably clear margin, and the posterior *P*(β \> 0) is close to one.
...
Consider a case in which the posterior mean of β is 100 and the 95% credible interval is \[40, 160\].
The interval tells us that we can be 95% certain the *true* value of β is between 40 and 160, given the data, our model, and our prior information.
