...
statistics. 
Specifically, this implies that we use Bayesian *credible intervals*---and 
other metrics---to draw statistical inferences.
...
(2) the highest density credible interval (HDI), (3) the proportion of the 
HDI that falls within a Region of Practical Equivalence (ROPE), and (4) the 
Maximum Probability of Effect (MPE).
...
@kruschke2018rejecting recommomends using the formula in \@ref(eq:rope)] 
The MPE calculates the proportion of the posterior distribution that is of the 
median's sign (or the probability that the effect is positive or negative). 
...
If, for instance, a hypothesis states that &beta; &gt; 0, we judge there to be 
...
Consider a case in which the posterior mean of &beta; is 100 and the 95% 
credible interval is [40, 160]. 
