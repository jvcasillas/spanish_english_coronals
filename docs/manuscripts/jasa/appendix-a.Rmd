...
For F1 and F2, 
`r f1_rope$b_language_sum`% and 
`r f1_rope$b_language_sum`% of the respective HDI's fell within the specified 
ROPE.
The probability that &beta; was greater than 0 was 
`r f1_mpe$b_language_sum` for F1 and 
`r f2_mpe$b_language_sum` for F2. 
...
In both cases 
`r f1_rope$b_phon_sum`% of the HDI fell within the ROPE and the probability 
that &beta; was less than 0 was 
`r f1_mpe$b_phon_sum` for F1 and 
`r f2_mpe$b_phon_sum` for F2.
...
A complete summary of the F1 and F2 models is available in 
...
read_csv(here("data", "tidy", "table_vowel_model_summary.csv"), na = "NA") %>% 
