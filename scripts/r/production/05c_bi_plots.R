...
bi_all_metrics <- bi_subj_item_means %>%
  ggplot(.) +
    aes(x = language, y = val, fill = phon, color = phon, shape = stress) +
    facet_wrap(~ metric, scales = "free_y",
               labeller = as_labeller(facet_labels)) +
    geom_beeswarm(dodge.width = 0.5, alpha = 0.3) +
    stat_pointinterval(data = posterior_bi_all, show.legend = F,
                       color = "black", .width = c(.80, .95),
                       position = position_dodge(0.5)) +
    stat_summary(data = posterior_bi_all, fun.y = mean, geom = "point",
...
path <- file.path(here("figs"), "bi_all_metrics.")
devices <- c('pdf', 'png')
