...
post_samples_FE_vot <- posterior_samples(mod_coronals_vot_mono_full) %>%
  as_tibble(.) %>%
  select(starts_with("b_"))

with(post_samples_FE_vot,
     mean((b_Intercept + b_phon_sum + b_group_sum) -
          (b_Intercept - b_phon_sum - b_group_sum) > 0))
